#!/usr/bin/env bash
# test-isolated.sh - Verify Kitsune.app runs without Homebrew or system Python
set -euo pipefail
cd "$(dirname "$0")/.."

APP="dist/Kitsune.app"
DMG="${DMG_PATH:-}"
...
if [ -z "$DMG" ]; then
    DMG="$(find dist -maxdepth 1 -name 'Kitsune-*.dmg' -type f -print 2>/dev/null \
        | sort | tail -1)"
fi

echo "==> Kitsune isolation test"
echo "    App:  $APP"
...
find_app_pids() {
    ...
    pgrep -f "$app_path/Contents/MacOS/kitsune" 2>/dev/null || true
}
...
make_pixbuf_cache() {
    ...
    local cache
    cache="$(TMPDIR="$TMPDIR_VALUE" mktemp -t kitsune-pixbuf-cache)"
...
run_webp_decode_probe() {
    local app_path="$1"
    local label="$2"
    local launcher="$app_path/Contents/MacOS/kitsune"
    ...
    py_ver="$(ls "$frameworks/Python.framework/Versions/" | grep -E '^3\.[0-9]+$' | head -1)"
    ...
    log="$(TMPDIR="$TMPDIR_VALUE" mktemp -t kitsune-webp-probe)"
    ...
    env -i \
        ...
        GST_PLUGIN_PATH="$resources/gstreamer-1.0" \
        GST_PLUGIN_SYSTEM_PATH="" \
        GST_PLUGIN_SCANNER="$resources/libexec/gstreamer-1.0/gst-plugin-scanner" \
        SSL_CERT_FILE="$resources/ssl/cert.pem" \
        "$py_bin" -c "
...
cleanup_keyring_entry() {
    env -i \
        ...
        GST_PLUGIN_SCANNER="$RESOURCES/libexec/gstreamer-1.0/gst-plugin-scanner" \
        SSL_CERT_FILE="$RESOURCES/ssl/cert.pem" \
        "$PY_BIN" -c "import keyring; keyring.delete_password('kitsune-isolation-test', 'test')" \
        >/dev/null 2>&1 || true
}
...
LAUNCHER="$APP/Contents/MacOS/kitsune"
DIR="$(cd "$(dirname "$LAUNCHER")" && pwd)"
BUNDLE="$(dirname "$DIR")"
RESOURCES="$BUNDLE/Resources"
FRAMEWORKS="$BUNDLE/Frameworks"
...
IMPORT_LOG="$(TMPDIR="$TMPDIR_VALUE" mktemp -t kitsune-import)"
set +e
env -i \
    HOME="$HOME" \
    TMPDIR="$TMPDIR_VALUE" \
...
gettext.bindtextdomain('kitsune', '$RESOURCES/locale')
gettext.textdomain('kitsune')
gettext.install('kitsune', '$RESOURCES/locale')
import gi
from gi.repository import Gio
gresource = Gio.Resource.load('$RESOURCES/app/net.armatik.Kitsune.gresource')
gresource._register()
...
print('GTK4+Adwaita+Soup+Gst: OK')
import keyring
...
keyring.set_password('kitsune-isolation-test', 'test', 'hello')
assert keyring.get_password('kitsune-isolation-test', 'test') == 'hello'
keyring.delete_password('kitsune-isolation-test', 'test')
print('keyring Keychain: read/write/delete OK')
import kitsune.application
print('kitsune.application: OK')
import base64, os, tempfile
gi.require_version('GdkPixbuf', '2.0')
from gi.repository import GdkPixbuf
payload = base64.b64decode('UklGRj4AAABXRUJQVlA4IDIAAAAQAgCdASoBAAEAAgA0JaACdLoB+AH6AAPIAP784yX/0AZLaqL/0Aj/9AGS2qi/8+oAAA==')
...
LAUNCH_LOG="$(TMPDIR="$TMPDIR_VALUE" mktemp -t kitsune-launcher)"
set +e
env -i \
    HOME="$HOME" \
    TMPDIR="$TMPDIR_VALUE" \
...
if [ -z "$DMG" ] || [ ! -f "$DMG" ]; then
    echo "FAIL: DMG not found. Expected dist/Kitsune-*.dmg or DMG_PATH=/path/to/file.dmg"
    exit 1
fi

ATTACH_LOG="$(TMPDIR="$TMPDIR_VALUE" mktemp -t kitsune-dmg-attach)"
set +e
...
OPEN_LOG="$(TMPDIR="$TMPDIR_VALUE" mktemp -t kitsune-open)"
set +e
...
if ! DMG_PID="$(wait_for_pid_from_app_path "$MOUNTED_APP")"; then
    fail_with_log "Could not find a running Kitsune process from mounted DMG" "$OPEN_LOG"
fi
